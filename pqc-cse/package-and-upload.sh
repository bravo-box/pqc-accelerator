...
# Target CPython minor versions the wheelhouse must support. Arc-managed Linux
# hosts ship whatever python3 their distro defaults to (e.g. RHEL8/9 -> 3.9,
...
build_wheelhouse() {
    ...
    for py_ver in "${PY_VERSIONS[@]}"; do
        ...
        log "  ...downloading wheels for Python ${py_ver} ($platform_tag)"
        "${pip_cmd[@]}"
    done
...
# Multiple CPython versions can yield the same dependency name with different
# versions, and the same wheel may appear more than once across build jobs.
# Keep one version per package. Select the lowest version present so the shared
# lock remains installable by the oldest supported interpreter (Python 3.9);
# newer package versions can have no compatible wheel for that interpreter.
...
for wheel in sorted(wheelhouse.glob("*.whl")):
    ...
    if current is None or LooseVersion(version) < LooseVersion(current["version"]):
        packages[key] = {"name": name, "version": version, "hashes": {sha}}
    elif LooseVersion(version) == LooseVersion(current["version"]):
        current["hashes"].add(sha)
...
with lock_file.open("w", encoding="utf-8") as f:
    ...
    for key in sorted(packages):
        pkg = packages[key]
        hash_args = " ".join(f"--hash=sha256:{h}" for h in sorted(pkg["hashes"]))
        f.write(f"{pkg['name']}=={pkg['version']} {hash_args}\n")
PY
}
